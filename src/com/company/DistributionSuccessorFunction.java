        ...
        for (int c = 0; c < Estat.camions.length; ++c) {
            ...
            for (int p = 0; p < Estat.peticions.length; ++p) {
                ...
                for (int v = 0; v < 5; ++v) {
                    ...
                    // FEM UN REPLACE DE UN VIATGE QUE JA ESTAT ATÉS , IMPOSEM LA NOSTRA  PETICIO
                    Estat new_state3 = new Estat(board.getDades_camio(), board.getPeticio_atesa());
                    if (new_state3.replace(c,v,p)) {
                        retval.add(new Successor(new String("repleace C:" + c + " P" + p + " V" + v), new_state3));
                    }
